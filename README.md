...
| `attempt/v4.1` | Selectable K′ + leftover toggle of the v4 origin. Lab default. Same 15.23 MB. |
...
v4.1 is the same origin as v4 with a live K′ peel and a leftover on/off at decode. Mean 33.95 dB at full leftover-on, 33.82 dB at K′=16 leftover-off, 20.9 dB at μ. Origin still 15.23 MB. `[` `]` peels rank, `L` toggles leftover. v4 stays frozen. Blockiness campaign: dense leftover stride 1 is the moving-element fix (not packed); hop-4 overlap is the lattice path.
