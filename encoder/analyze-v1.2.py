#!/usr/bin/env python3
"""narc attempt v1.2 — same 16×16 stack as v1.1, with a geometric leftover model.

Shot-level affine (scale / rotate / shear / translate) replaces the single
translation. Local MVs are sub-pel corrections. Median only snaps isolated
outliers so a zoom or parallax field can survive. No nets. No CU tree.
v0 / v1 / v1.1 media are frozen under public/media/v{0,1,1.1}.
"""
...
def encode(rgbs: list[np.ndarray]) -> dict:
    ...
    for i, rgb in enumerate(rgbs):
        ...
        if want_key:
            ...
            motion_bytes += 24 + 4 + (n_blocks + 7) // 8 + (n_resid + n_intra) * 2
            is_key = False
        ...
        if i % 120 == 0:
            print(
                ...
                f"skip={n_skip} resid={n_resid} intra={n_intra} "
...
def main() -> None:
    ...
    (MEDIA / "stats-v1.2.json").write_text(json.dumps(data["stats"], indent=2))
