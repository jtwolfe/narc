#!/usr/bin/env python3
"""narc attempt v3 — CU tree + real bitstream on the v2 stack.

Same 16×16 affine + sub-pel warp. Tiny field nets stay on smooth leftover.
Residual 16×16 may JPEG; 8×8 / 4×4 residual is DCT + quant + run-level.
Syntax is Exp-Golomb then zlib. Two byte meters + a gzip-v2-side-info control.
No clip-wide NeRV. No 1×1 CUs.
v0 / v1 / v1.1 / v1.2 / v2 media are frozen under public/media/v{0,1,1.1,1.2,2}.
"""
...
NET_DC_BYTES = 6.0
NET_LIN_BYTES = 14.0
SPLIT_QSTD = 1.15  # consider skip-split when 8×8 quadrants disagree
...
def _dct_matrix(n: int) -> np.ndarray:
    ...
    for k in range(n):
        ...
        for i in range(n):
            C[k, i] = a * math.cos(math.pi * (2 * i + 1) * k / (2.0 * n))
    return C


C8 = _dct_matrix(8)
C4 = _dct_matrix(4)
# Residual quant — finer than JPEG q=52 luma so PSNR can hold; RDO still refuses
# empty / textured-unsplit leaves.
Q8 = np.array(
    [
        [6, 5, 5, 6, 8, 10, 12, 14],
        [5, 5, 6, 7, 8, 12, 13, 12],
        [5, 6, 6, 8, 10, 12, 14, 12],
        [6, 7, 8, 10, 12, 16, 16, 14],
        [8, 8, 10, 12, 14, 18, 18, 16],
        [10, 12, 12, 14, 16, 18, 20, 18],
        [12, 14, 16, 16, 18, 20, 20, 18],
        [14, 16, 16, 18, 20, 18, 18, 18],
    ],
    dtype=np.float32,
)
Q4 = np.array(
...
ZZ4 = np.array([0, 1, 4, 8, 5, 2, 3, 6, 9, 12, 13, 10, 7, 11, 14, 15], dtype=np.int32)
...
def fit_patch_nets(pred: np.ndarray, cur: np.ndarray, modes: np.ndarray) -> tuple[np.ndarray, np.ndarray, int, int, bytes]:
    ...
    if n_net:
        ...
        blob = struct.pack("<f", scale) + kinds.astype(np.uint8).tobytes() + q.tobytes()

    delta = _scatter_tiles(delta_tiles, pred.shape)
    # residual-mode blocks taken by the net should not also pay JPEG
    return delta, kinds.reshape(ROWS, COLS), nbytes, n_net, blob
...
def ue_bits(n: int) -> int:
    x = max(int(n), 0) + 1
    return 2 * x.bit_length() - 1


def se_bits(n: int) -> int:
    n = int(n)
    if n <= 0:
        return ue_bits(-2 * n)
    return ue_bits(2 * n - 1)
...
class BitWriter:
    ...
    def write_ue(self, n: int) -> None:
        x = max(int(n), 0) + 1
        k = x.bit_length()
        self.write_bits(0, k - 1)
        self.write_bits(x, k)

    def write_se(self, n: int) -> None:
        n = int(n)
        if n <= 0:
            self.write_ue(-2 * n)
        else:
...
def _run_level(q: np.ndarray, zz: np.ndarray) -> list[tuple[int, int]]:
    flat = q.ravel()[zz]
    last = int(flat.size)
    while last > 0 and int(flat[last - 1]) == 0:
        last -= 1
    run = 0
...
def _pairs_bits(pairs: list[tuple[int, int]]) -> int:
    b = ue_bits(len(pairs))
    for run, level in pairs:
        b += ue_bits(run) + se_bits(level)
    return b


def dct_encode_tile(res: np.ndarray, n: int) -> tuple[np.ndarray, list[list[tuple[int, int]]], int]:
    """n×n×3 float residual → (recon residual, per-channel run-level, bits)."""
    C = C8 if n == 8 else C4
    Q = Q8 if n == 8 else Q4
    zz = ZZ8 if n == 8 else ZZ4
    rec = np.zeros((n, n, 3), dtype=np.float32)
    all_pairs: list[list[tuple[int, int]]] = []
    bits = 0
    ct = C.T
    for ch in range(3):
        block = res[:, :, ch].astype(np.float32)
        coeff = C @ block @ ct
        q = np.round(coeff / Q).astype(np.int32)
        rec[:, :, ch] = ct @ (q.astype(np.float32) * Q) @ C
        pairs = _run_level(q, zz)
        all_pairs.append(pairs)
        bits += _pairs_bits(pairs)
    return rec, all_pairs, bits


def _write_pairs(w: BitWriter, pairs_rgb: list[list[tuple[int, int]]]) -> None:
    for pairs in pairs_rgb:
        w.write_ue(len(pairs))
        for run, level in pairs:
            w.write_ue(run)
            w.write_se(level)
...
def rdo_split_16(
    pred_t: np.ndarray, cur_t: np.ndarray, is_residual: bool
) -> tuple[bool, np.ndarray | None, list | None, float]:
    """RDO-split a 16×16 residual (or high-var skip) into 8×8 / 4×4 DCT leaves."""
    res = cur_t.astype(np.float32) - pred_t.astype(np.float32)
    mae_rgb = float(np.abs(res).mean())
    if is_residual:
        j_un = min(mae_rgb * 0.22 + 1.6, mae_rgb) + LAMBDA * 28.0
    else:
        j_un = mae_rgb

    def encode_sq(y0: int, x0: int, n: int) -> tuple[np.ndarray, list, int, float]:
        rec, pairs, bits = dct_encode_tile(res[y0 : y0 + n, x0 : x0 + n], n)
        d = float(np.abs(rec - res[y0 : y0 + n, x0 : x0 + n]).mean())
        j = d + LAMBDA * (bits / 8.0)
        return rec, pairs, bits, j

    children: list[dict] = []
    rec16 = np.zeros_like(res)
    j_split = LAMBDA * (1.0 / 8.0)  # 16×16 split flag
    for y0, x0 in ((0, 0), (0, 8), (8, 0), (8, 8)):
        rec8, pairs8, bits8, j8 = encode_sq(y0, x0, 8)
        rec4s = np.zeros((8, 8, 3), dtype=np.float32)
        kids4: list[dict] = []
        j4 = LAMBDA * (1.0 / 8.0)
        bits4 = 1
        for dy, dx in ((0, 0), (0, 4), (4, 0), (4, 4)):
            rec4, p4, b4, j4i = encode_sq(y0 + dy, x0 + dx, 4)
            rec4s[dy : dy + 4, dx : dx + 4] = rec4
            kids4.append({"y": y0 + dy, "x": x0 + dx, "pairs": p4, "bits": b4})
            j4 += j4i
            bits4 += b4
        if j4 + 0.05 < j8:
            rec16[y0 : y0 + 8, x0 : x0 + 8] = rec4s
            children.append({"split": True, "y": y0, "x": x0, "kids": kids4, "bits": bits4})
            j_split += j4
        else:
            rec16[y0 : y0 + 8, x0 : x0 + 8] = rec8
            children.append({"split": False, "y": y0, "x": x0, "pairs": pairs8, "bits": bits8})
            j_split += j8

    if j_split + 0.12 < j_un:
        return True, rec16, children, j_split
    return False, None, None, j_un


def write_cu_tree(w: BitWriter, children: list[dict]) -> None:
    for kid in children:
        w.write_bit(1 if kid["split"] else 0)
        if kid["split"]:
            for k4 in kid["kids"]:
                _write_pairs(w, k4["pairs"])
        else:
            _write_pairs(w, kid["pairs"])


def quadrant_std(err: np.ndarray) -> np.ndarray:
    """Per 16×16 std of the four 8×8 mean abs errors. err is HxW."""
    e = err[: ROWS * BH, : COLS * BW]
    q = e.reshape(ROWS, 2, 8, COLS, 2, 8).mean(axis=(2, 5))
    return q.reshape(ROWS, COLS, 4).std(axis=2)


def write_pred_syntax(
    w: BitWriter,
    affine: Affine,
    ref_used: int,
    ldx: np.ndarray,
    ...
    resid_m: np.ndarray,
    intra_m: np.ndarray,
    net_kinds: np.ndarray,
    split_m: np.ndarray,
    trees: dict[tuple[int, int], list],
    net_blob: bytes,
) -> None:
    w.write_bit(0)  # predicted
    w.write_bit(int(ref_used) & 1)
    a, b, c, d, e, f = affine
    w.write_se(int(round(a * 4096)))
    w.write_se(int(round(b * 4096)))
    w.write_se(int(round(c * 16)))
    w.write_se(int(round(d * 4096)))
    w.write_se(int(round(e * 4096)))
    w.write_se(int(round(f * 16)))
    qdx = np.round(ldx.astype(np.float32) * 4.0).astype(np.int32)
    qdy = np.round(ldy.astype(np.float32) * 4.0).astype(np.int32)
    for r in range(ROWS):
        for col in range(COLS):
            if intra_m[r, col]:
                mode = 2
            elif net_kinds[r, col] > 0:
                mode = 3
            elif split_m[r, col] or resid_m[r, col]:
                mode = 1
            else:
                mode = 0
            w.write_bits(mode, 2)
            ...
            if r > 0:
                ...
                npred += 1
            if r > 0 and col > 0:
                sx += int(qdx[r - 1, col - 1])
                sy += int(qdy[r - 1, col - 1])
                npred += 1
            if npred:
                px, py = sx // npred, sy // npred
            w.write_se(int(qdx[r, col]) - px)
            w.write_se(int(qdy[r, col]) - py)
            if mode == 1:
                w.write_bit(1 if split_m[r, col] else 0)
                if split_m[r, col]:
                    write_cu_tree(w, trees[(r, col)])
            elif mode == 3:
                w.write_bit(0 if int(net_kinds[r, col]) == 1 else 1)
    if net_blob:
        w.write_bit(1)
        w.write_ue(len(net_blob))
        w.write_bytes(net_blob)
    else:
        w.write_bit(0)


def v2_sideinfo_blob(
    affine: Affine,
    skip_m: np.ndarray,
    ldx: np.ndarray,
    ldy: np.ndarray,
    ...
    buf = bytearray(struct.pack("<6f", *[float(x) for x in affine]))
    bits = np.packbits(skip_m.astype(np.uint8).ravel())
    buf += bits.tobytes()
    qdx = np.round(ldx.astype(np.float32) * 4.0).astype(np.int16)
    qdy = np.round(ldy.astype(np.float32) * 4.0).astype(np.int16)
    for r, c in np.argwhere(coded):
        buf += struct.pack("<hh", int(qdx[r, c]), int(qdy[r, c]))
    buf += net_blob
    return bytes(buf)
...
def deblock(img: np.ndarray, dx: np.ndarray, dy: np.ndarray, modes: np.ndarray, split_m: np.ndarray | None = None) -> np.ndarray:
    ...
    if split_m is not None:
        for r in range(ROWS):
            for c in range(COLS):
                ...
                y0, x0 = r * BH, c * BW
                x = x0 + 8
                left = out[y0 : y0 + 16, x - 1]
                right = out[y0 : y0 + 16, x]
                out[y0 : y0 + 16, x - 1] = (2.0 * left + right) / 3.0
                out[y0 : y0 + 16, x] = (2.0 * right + left) / 3.0
                y = y0 + 8
                up = out[y - 1, x0 : x0 + 16]
                down = out[y, x0 : x0 + 16]
                out[y - 1, x0 : x0 + 16] = (2.0 * up + down) / 3.0
                out[y, x0 : x0 + 16] = (2.0 * down + up) / 3.0
    return np.clip(out, 0, 255).astype(np.uint8)
...
def self_test() -> None:
    ...
    delta, kinds, _, nnet, _ = fit_patch_nets(pred_n, cur_n, modes_n)
    ...
    d2, k2, _, n2, _ = fit_patch_nets(pred_n, cur_g, modes_n)
    ...
    # DCT roundtrip on an 8×8 ramp
    ramp = np.linspace(-18, 18, 8, dtype=np.float32)
    tile = np.zeros((8, 8, 3), dtype=np.float32)
    tile[..., 0] = ramp[None, :]
    rec_d, pairs_d, bits_d = dct_encode_tile(tile, 8)
    mae_d = float(np.abs(rec_d - tile).mean())
    if mae_d > 2.8:
        raise SystemExit(f"self-test FAILED: 8×8 DCT roundtrip mae {mae_d:.3f} bits={bits_d}")
    tw = BitWriter()
    _write_pairs(tw, pairs_d)
    packed = tw.finish()
    tr = BitReader(packed)
    for ch_pairs in pairs_d:
        n_p = tr.read_ue()
        got = [(tr.read_ue(), tr.read_se()) for _ in range(n_p)]
        if got != ch_pairs:
            raise SystemExit("self-test FAILED: run-level bitstream did not roundtrip")
    ...
    mw.write_se(7)
    mw.write_ue(4)
    mr = BitReader(mw.finish())
    if mr.read_se() != -3 or mr.read_se() != 7 or mr.read_ue() != 4:
        raise SystemExit("self-test FAILED: MV exp-golomb roundtrip")

    # two-depth 16×16 must split and beat unsplit JPEG-ish J
    pred_t = np.full((16, 16, 3), 80, dtype=np.uint8)
    cur_t = pred_t.copy()
    cur_t[:, :8] = 110
    did_split, rec_res, kids, j_s = rdo_split_16(pred_t, cur_t, True)
    if not did_split or rec_res is None:
        raise SystemExit("self-test FAILED: two-depth 16×16 did not split")
    rec_s = np.clip(pred_t.astype(np.float32) + rec_res, 0, 255)
    mae_s = float(np.abs(rec_s - cur_t.astype(np.float32)).mean())
    mae_s_id = float(np.abs(pred_t.astype(np.float32) - cur_t.astype(np.float32)).mean())
    if mae_s >= mae_s_id * 0.45:
        raise SystemExit(f"self-test FAILED: split mae {mae_s:.3f} not better than {mae_s_id:.3f}")
    n8 = sum(1 for k in kids or [] if not k["split"])
    n4 = sum(4 for k in kids or [] if k["split"])
    if n8 + n4 < 4:
        raise SystemExit(f"self-test FAILED: split produced no leaves n8={n8} n4={n4}")
    ...
    print(
        ...
        f"dct_mae={mae_d:.3f} split_mae={mae_s:.3f} split_j={j_s:.3f}"
    )
...
def encode(rgbs: list[np.ndarray]) -> dict:
    ...
    split_total = cu8_total = cu4_total = cu16_total = 0
    n_blocks = ROWS * COLS
    black_frames = 0
    scales: list[float] = []
    syntax = BitWriter()
    ctrl_blobs: list[bytes] = []
    ...
    for i, rgb in enumerate(rgbs):
        ...
        stored = False
        n_split = n_cu8 = n_cu4 = 0
        heat_leaves: list[tuple[int, int, int]] = []
        split_m = np.zeros((ROWS, COLS), dtype=bool)
        net_kinds = np.zeros((ROWS, COLS), dtype=np.uint8)
        if want_key:
            rec_path = KEYS / f"{i:04d}.jpg"
            key_bytes += save_jpg(rec_path, rgb, KEY_JPEG_Q)
            rec = pad_frame(np.array(Image.open(rec_path).convert("RGB")))
            ...
            net_delta, net_kinds, net_b, n_net, net_blob = fit_patch_nets(pred, rgb, modes)
            rec = apply_patch_delta(pred, net_delta)
            net_bytes += net_b
            resid_m = (modes == 1) & (net_kinds == 0)
            skip_m = (modes == 0) & (net_kinds == 0)
            intra_m = modes == 2
            err_rgb = np.abs(rgb.astype(np.float32) - rec.astype(np.float32)).mean(axis=2)
            qstd = quadrant_std(err_rgb)
            consider = resid_m | (skip_m & (qstd >= SPLIT_QSTD))
            trees: dict[tuple[int, int], list] = {}
            dct_delta = np.zeros_like(rec, dtype=np.float32)
            for rr, cc in np.argwhere(consider):
                r_i, c_i = int(rr), int(cc)
                y0, x0 = r_i * BH, c_i * BW
                did, rec_res, kids, _j = rdo_split_16(
                    rec[y0 : y0 + 16, x0 : x0 + 16],
                    rgb[y0 : y0 + 16, x0 : x0 + 16],
                    bool(resid_m[r_i, c_i]),
                )
                if not did or rec_res is None or kids is None:
                    continue
                dct_delta[y0 : y0 + 16, x0 : x0 + 16] = rec_res
                split_m[r_i, c_i] = True
                trees[(r_i, c_i)] = kids
                skip_m[r_i, c_i] = False
                resid_m[r_i, c_i] = False
                n_split += 1
                for kid in kids:
                    if kid["split"]:
                        n_cu4 += 4
                        for k4 in kid["kids"]:
                            heat_leaves.append((k4["y"] + y0, k4["x"] + x0, 4))
                    else:
                        n_cu8 += 1
                        heat_leaves.append((kid["y"] + y0, kid["x"] + x0, 8))
            if n_split:
                rec = np.clip(rec.astype(np.float32) + dct_delta, 0, 255).astype(np.uint8)
            n_resid = int(resid_m.sum())
            ...
            if n_intra > 0:
                ...
                intra_bytes += b
            rec = deblock(rec, cand["ldx"], cand["ldy"], modes, split_m)
            n_coded = int((resid_m | intra_m | (net_kinds > 0) | split_m).sum())
            motion_bytes += 24 + 4 + (n_blocks + 7) // 8 + n_coded * 2 + (n_split + 7) // 8
            write_pred_syntax(
                syntax,
                cand["affine"],
                ref_used,
            ...
            ctrl_blobs.append(
                v2_sideinfo_blob(
                    ...
                    cand["ldx"],
                    cand["ldy"],
                    (resid_m | intra_m | (net_kinds > 0) | split_m),
                    net_blob,
                )
            )
            is_key = False
        ...
        if i % 120 == 0:
            print(
                ...
                f"split={n_split} cu8={n_cu8} cu4={n_cu4} "
        ...
        if is_key or cut or residual > 10:
            ...
            if not is_key:
                ...
                heat_modes[split_m] = 4
            draw_heat(rgb, pred if not is_key else rgb, heat_modes, i, heat_leaves)
    ...
    v2 = load_stats(MEDIA / "v2" / "stats.json") or load_stats(MEDIA / "stats-v2.json")
    syntax_raw = syntax.finish()
    syntax_zlib = zlib.compress(syntax_raw, 9)
    ctrl = b"".join(ctrl_blobs)
    gzip_control = len(zlib.compress(ctrl, 9)) if ctrl else 0
    bitstream_bytes = len(syntax_zlib) + key_bytes + resid_bytes + intra_bytes
    raw_accounted = key_bytes + resid_bytes + motion_bytes + intra_bytes + net_bytes
    model_bytes = bitstream_bytes
    kinds: dict[str, int] = {}
    for r in rows:
    ...
    stats = {
        ...
        "netBytes": net_bytes,
        "bitstreamBytes": bitstream_bytes,
        "rawAccountedBytes": raw_accounted,
        "gzipControlBytes": gzip_control,
        "syntaxBytes": len(syntax_raw),
        "syntaxZlibBytes": len(syntax_zlib),
        ...
        "splitFrac": round(split_total / (n * n_blocks), 4),
        "cu16Count": int(cu16_total),
        "cu8Count": int(cu8_total),
        "cu4Count": int(cu4_total),
        ...
        "baselineV2": slim(v2, "v2-residual-nets"),
    }
    return {
        "attempt": ATTEMPT,
        "frames": rows,
...
def draw_heat(
    ...
    modes: np.ndarray,
    i: int,
    leaves: list[tuple[int, int, int]] | None = None,
) -> None:
    ...
    for y0, x0, sz in leaves or []:
        if y0 >= H_DISP:
            continue
        color = (220, 180, 100) if sz == 4 else (180, 140, 80)
        y1 = min(y0 + sz, H_DISP)
        x1 = min(x0 + sz, W)
        vis[y0 : y0 + 1, x0:x1] = color
        if y1 - 1 >= y0:
            vis[y1 - 1 : y1, x0:x1] = color
        vis[y0:y1, x0 : x0 + 1] = color
        vis[y0:y1, x1 - 1 : x1] = color
...
def main() -> None:
    ...
    (MEDIA / "stats-v3.json").write_text(json.dumps(data["stats"], indent=2))
